...
declare type Listener<TPayload> = (payload: TPayload) => void;
declare class Subscribable<TPayload = void> {
...
declare class MatchLoader<TGenerics extends PartialGenerics = DefaultGenerics> extends Subscribable<undefined | boolean> {
    router: RouterInstance<TGenerics>;
    location: Location<TGenerics>;
    matches: RouteMatch<TGenerics>[];
...
export declare const Link: <TGenerics extends Partial<DefaultGenerics> = DefaultGenerics>({ to, search, hash, children, target, style, replace, onClick, onMouseEnter, className, getActiveProps, activeOptions, preload, disabled, _ref, ...rest }: LinkProps<TGenerics>) => JSX.Element;
